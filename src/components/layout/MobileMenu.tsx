...
export function MobileMenu({ links }: MobileMenuProps) {
  ...
  return (
    <>
      ...
      <button
        onClick={open}
        className="flex items-center p-1 md:hidden"
      ...
      {/* Drawer */}
      <div
        className={`fixed right-0 top-0 z-[70] flex h-full w-full max-w-sm flex-col bg-background p-6 shadow-xl transition-transform duration-300 ease-in-out md:hidden ${
        ...
        <nav className="flex flex-col gap-2">
          {links.map((link) => (
            <a
              ...
              href={link.href}
              onClick={close}
              className="border-b border-border pb-3 text-xl font-semibold text-foreground transition-colors hover:text-primary"
            >
              {link.label}
            </a>
          ))}
