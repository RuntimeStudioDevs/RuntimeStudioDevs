    ...
    >
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .nav-link-hover::after {
          ...
          width: 0;
          height: 2px;
          background-color: var(--color-studio-blue, #008cf9);
      ...
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 md:px-8">
        ...
        <nav className="hidden items-center gap-6 md:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="nav-link-hover text-sm font-medium text-foreground/80 transition-colors hover:text-primary"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="hidden items-center gap-4 md:flex">
          ...
          <a
            href="#contacto"
            className="rounded-full bg-primary px-5 py-2 text-sm font-semibold text-white transition-all hover:bg-primary/90 active:scale-95"
          >
            Contáctanos
          </a>
        </div>
